...
function getCoordinates(position) {
  fetch(
    `https://us1.locationiq.com/v1/reverse.php?key=pk.edb81f0c3da10cab4bd7574d25e013b9&format=json&lat=${position.coords.latitude}&lon=${position.coords.longitude}`
  )
    .then((response) => {
...
function getWeatherForToday(city) {
  fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=3a1ec32886627a2b874bbd5495988c71&units=metric`
  )
    .then((response) => {
      return response.json();
    })
    .then((myJson) => {
      ...
      for (let index = 0; index < temperature.toFixed(1).length; index++) {
        ...
        newIcon.classList.add(
          `${mapTemperature[temperature.toFixed(1)[index]]}`
        );
...
function errorHandling(error) {
  ...
  switch (error.code) {
    case error.PERMISSION_DENIED:
      console.log(
        `"User denied the request for Geolocation, using default city '${defaultCity}'"`
      );
      break;
    case error.POSITION_UNAVAILABLE:
      console.log(
        `"Location information is unavailable, using default city '${defaultCity}'"`
      );
      break;
    case error.TIMEOUT:
      console.log(
        `"The request to get user location timed out, using default city '${defaultCity}'"`
      );
      break;
